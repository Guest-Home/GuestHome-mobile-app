...
import '../../../../../../auth/domain/entities/verify_otp_entity.dart';
...
class ChangePhoneBloc extends Bloc<ChangePhoneEvent, ChangePhoneState> {
  ChangePhoneBloc() : super(ChangePhoneInitial()) {
    on<GetOtpForOldPhoneEvent>((event, emit)async{
      ...
      response.fold((l) => emit(PhoneChangeErrorState(state, failure: l)),
            (r) => emit(GettingOtpOldPhoneSuccess(state, otpResponseEntity: r)),);
    });
    on<AddOldOtpEvent>((event, emit) => emit(state.copyWith(oldOtp: event.otp)),);
    on<AddNewOtpEvent>((event, emit) => emit(state.copyWith(newOtp: event.otp)),);
    on<VerifyOldOtpEvent>((event, emit)async{
      ...
      response.fold((l) => emit(PhoneChangeErrorState(state, failure: l)),(r) =>
        emit(VerifyingOtpOldSuccess(state,message: r)),);
    },);
    on<GetOtpForNewPhoneEvent>((event, emit) async{
    ...
    on<VerifyNewOtpEvent>((event, emit)async{
      ...
      response.fold((l) => emit(PhoneChangeErrorState(state, failure: l)),(r) =>
          emit(VerifyingOtpNewSuccess(state,message: r)),);
    },);
  }
}
